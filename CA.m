...
%         
        ...
        if abs(h_desired_usv - pre_h_usv) > 15*pi/180
           h_usv = pre_h_usv + sign(h_desired_usv - pre_h_usv)*15*pi/180;
    ...
    %% If no collision, go back to origional path
     if status_flag == 0
         ...
         if abs(h_usv_back - pre_h_usv) > 15*pi/180
            h_usv = pre_h_usv + sign(h_usv_back - pre_h_usv)*15*pi/180;
    ...
    for i = 1:n_intruder
        [x_intruder_one_plot, y_intruder_one_plot, H_intruder(i), G_intruder(i)] =draw_intruder(x_shape, y_shape, x_intruder(i), y_intruder(i), scale, h_intruder(i));
    end
    
    plot(x_usv, y_usv,'.','MarkerEdgeColor','r')
    hold on
